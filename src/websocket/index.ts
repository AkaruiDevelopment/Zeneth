  ...
  #handleEvents() {
    ...
    this.ws.on("close", (code, msg) => {
      ...
      if (canReconnectOnCodes[code]) this.#reconnect();
        ...
        canReconnectOnCodes[code as keyof typeof canReconnectOnCodes] ===
        undefined
      )
        this.#reconnect();
      else if (
        canReconnectOnCodes[code as keyof typeof canReconnectOnCodes] === false
      )
        throw ZenethError.WebSocketError(
          errorCodeMessages[code as keyof typeof canReconnectOnCodes] ??
            "Unknown Error",
          code
        );
    ...
    const data = {
      ...
      d: {
        ...
        properties: {
          ...
          browser:
            this.#client.options.identify?.properties?.browser ?? "Uzumi",
