    ...
    #handleEvents() {
        ...
        this.ws.on("close", (code, msg) => {
            ...
            if (canReconnectOnCodes[code])
                this.#reconnect();
            else if (canReconnectOnCodes[code] ===
                undefined)
                this.#reconnect();
            else if (canReconnectOnCodes[code] === false)
                throw ZenethError.WebSocketError(errorCodeMessages[code] ??
                    "Unknown Error", code);
            else
                throw ZenethError.WebSocketError(msg.toString(), code);
        });
